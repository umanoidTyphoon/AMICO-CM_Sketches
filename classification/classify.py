...
output_file_sketch = "test_sketch.arff"

def print_arff_from_sketch(dump_id):
    conn = util.connect_to_db()
    cursor = conn.cursor(cursor_factory=psycopg2.extras.NamedTupleCursor)
    cursor.execute("""
        SELECT * FROM weka_features_sketch
        WHERE dump_id = %s""",
        (dump_id, ))
...
def classify_dump(dump_id):
    print_arff(dump_id)
    print_arff_from_sketch(dump_id)
    subprocess.call(
    ...
    subprocess.call(
            "java -Xmx2000m -cp ./weka.jar "
            "weka.classifiers.meta.FilteredClassifier "
            "-l %s -p 1,58,59 -distribution -T test_sketch.arff "
            "> test_sketch.result" % ("default.model",), shell=True)
    ...
    with open('test_sketch.result', 'r') as f:
        for line in f:
            if ':' in line:
                for word in line.split():
                    if '*' in word:
                        score_sketch = word.split(',')[0]
                        if score_sketch.startswith('*'):
                            score_sketch = score_sketch[1:]

    print "AMICO Score sketch:", score_sketch
    ...
    subprocess.call("rm test_sketch.arff", shell=True)
    subprocess.call("rm test_sketch.result", shell=True)
